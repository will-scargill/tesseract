...
@bp.route("/admin/network", methods=["GET"])
def adminNetwork():
    """ Placeholder """
    if "user" in session:
        found_users = users.query.filter_by(name=session["user"]).first()
...
@bp.route("/admin/accounts/toggle/<userid>", methods=["GET"])
...
@bp.route("/admin/accounts/delete/<userid>", methods=["GET"])
...
@bp.route("/admin/links/delete/<linkid>", methods=["GET"])
...
@bp.route("/admin/accounts/change/<userid>", methods=["GET", "POST"])
